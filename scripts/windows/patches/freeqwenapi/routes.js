...
import { parseToolCallJson, remapToolCallNames } from './toolParser.js';
    ...
    try {
        ...
        if (stream) {
            ...
            try {
                ...
                if (captureToolCalls) {
                    const toolCalls = remapToolCallNames(parseToolCallJson(result?.choices?.[0]?.message?.content), combinedTools);
...
router.post('/v1/chat/completions', async (req, res) => {
    try {
        ...
        if (stream) {
            ...
            const toolCalls = remapToolCallNames(parseToolCallJson(messageText), combinedTools);
            if (toolCalls && toolCalls.length > 0) {
                return res.json(buildOpenAIToolResponse(result, mappedModel, toolCalls));
            }
