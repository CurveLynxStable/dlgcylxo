...
const FOREIGN_TOOL_KEYWORDS = {
    ls: ['list_dir', 'list', 'dir'],
    list: ['list_dir', 'list', 'dir'],
    glob: ['glob', 'list_dir', 'find', 'list', 'search'],
    find: ['find', 'glob', 'search', 'list'],
    read: ['read_file', 'read', 'view', 'open', 'file'],
    cat: ['read_file', 'read', 'view'],
    view: ['view', 'read'],
    grep: ['grep', 'search'],
    search: ['search', 'grep'],
    websearch: ['web_search', 'web'],
    webfetch: ['fetch', 'web'],
    bash: ['run_command', 'terminal', 'command', 'shell', 'run', 'bash', 'exec'],
    shell: ['run_command', 'terminal', 'command', 'shell', 'run', 'exec'],
    runcommand: ['run_command', 'terminal', 'command', 'shell', 'run', 'exec'],
    terminal: ['terminal', 'run_command', 'command', 'shell', 'run'],
    exec: ['run_command', 'terminal', 'command', 'exec', 'run'],
    write: ['write_file', 'write', 'create_file', 'create', 'edit'],
    edit: ['edit_file', 'edit', 'write', 'patch', 'replace'],
    patch: ['patch', 'edit', 'write'],
    todowrite: ['todo'],
    todoread: ['todo'],
    task: ['delegate', 'task', 'agent']
};
...
export function remapToolCallNames(toolCalls, tools) {
    if (!Array.isArray(toolCalls) || toolCalls.length === 0) return toolCalls;
    const available = (Array.isArray(tools) ? tools : [])
        .map(tool => tool?.function?.name || tool?.name)
        .filter(Boolean);
    if (available.length === 0) return toolCalls;
    const availableSet = new Set(available);
    const availableByKey = new Map(available.map(name => [normalizeToolKey(name), name]));

    return toolCalls.map(call => {
        const name = call?.function?.name;
        if (!name || availableSet.has(name)) return call;

        const exactKey = availableByKey.get(normalizeToolKey(name));
        if (exactKey) return { ...call, function: { ...call.function, name: exactKey } };

        const keywords = FOREIGN_TOOL_KEYWORDS[normalizeToolKey(name)];
        if (keywords) {
            for (const keyword of keywords) {
                const match = available.find(candidate => candidate.toLowerCase().includes(keyword));
                if (match) return { ...call, function: { ...call.function, name: match } };
            }
        }
        return call;
    });
